'''
    Amazon Cloud Drive XBMC Plugin
...
if mode == 'dummy' or mode == 'delete' or mode == 'enroll':
    ...
    invokedUsername = cloudservice.accountActions(addon, PLUGIN_NAME, mode, instanceName, numberOfAccounts)
                ...
                while loop:
                    ...
                    try:
                        ...
                        if username == invokedUsername:
                            ...
                            #let's log in
                            service = acd_api.acd(PLUGIN_URL,addon,instanceName, user_agent, settings)

                            loop = False
                    except:
                        service = acd.acd(PLUGIN_URL,addon,instanceName, user_agent)
                        break
                    ...
                    if count == numberOfAccounts:
                        try:
                            ...
                            service = acd_api.acd(PLUGIN_URL,addon,PLUGIN_NAME+'1', user_agent, settings)
                        break
                    count = count + 1
...
if instanceName is None and (mode == 'index' or mode == 'main'):
    ...
    service = acd_api.acd(PLUGIN_URL,addon,'', user_agent, settings, authenticate=False)
else:
    service = acd_api.acd(PLUGIN_URL,addon,instanceName, user_agent, settings)
...
#    encfs = settings.getParameter('encfs', False)
    ...
    # display option for all Videos/Music/Photos, across acd
    #** acd specific
    if mode == 'main':
...
#                    service.addDirectory(folder.folder(folderName,'[decrypted]'), contextType, encfs=True)
    ...
    try:
        ...
        xbmc.log(addon.getLocalizedString(30050)+ 'acd-login', xbmc.LOGERROR)
        xbmcplugin.endOfDirectory(plugin_handle)
