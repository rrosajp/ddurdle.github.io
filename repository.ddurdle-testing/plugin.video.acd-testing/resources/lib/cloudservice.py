...
def accountActions(addon, PLUGIN_NAME, mode, instanceName, numberOfAccounts):
    ...
    # enroll a new account
            ...
            if code == '':
                options = []
                options.append('Amazon Cloud Drive Testing OAUTH2')
                ...
                if ret == 0:
                    try:
                        ...
                        xbmcgui.Dialog().ok(addon.getLocalizedString(30000), addon.getLocalizedString(30206))
                        invokedUsername = dialog.input('username', type=xbmcgui.INPUT_ALPHANUM)
                        xbmcgui.Dialog().ok(addon.getLocalizedString(30000), addon.getLocalizedString(30207))
                ...
                xbmcgui.Dialog().ok(addon.getLocalizedString(30000), addon.getLocalizedString(30208), addon.getLocalizedString(30209), addon.getLocalizedString(30210))
                return invokedUsername
            else:
                count = 1
                loop = True
                while loop:
                    ...
                    try:
                        username = settings.getSetting(instanceName+'_username')
                        if username == invokedUsername:
                            addon.setSetting(instanceName + '_type', str(0))
